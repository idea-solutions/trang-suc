...
Validator.isEmail = function (selector ,message) {
    return {
        ...
        test(value) {
            var re = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
            return  re.test(value.trim()) ? undefined :message || "Vui lòng nhập trường này";
        }
    }
}
...
Validator.isPassword = function (selector,message , min) {
    return {
        selector : selector,
        test(value) {
            return  value.trim().length >= min ? undefined : message || "Vui lòng nhập trường này";
        }
    }
}
    ...
    return {
        ...
        test(value) {
            if(value.trim().length <= 0) {
                ...
                return  (value.trim() == confirmValue().trim())  ? undefined : message || "Vui lòng nhập trường này";
            }
        }
    }
}
